...
var custom_audio_player = {
  ...
  create_basic_controls: function (audio_player) {
    ...
    htmlString =
      '<div class="progress_control"><button id="play-icon"></button><span id="current-time" class="time">0:00/0:00</span><input type="range" id="seek-slider" max="100" value="0"></div><div class="volume_control"><button id="mute-icon"></button><input type="range" id="volume-slider" max="100" value="100"></div>';
  ...
  create_advanced_controls: function (audio_player) {
    ...
    htmlString =
      '<div class="speed-control"><div class="speed-input" style="margin-left: 10px"><label for="pbrate"><img src="icon/speed.png" style="float: left;" title="Velocidad" alt="" /></label><input type="range" id="pbrate" min=0.005 max=2.0 value=1.0 step=.0001></div><div class="speed-display" style="font-family: "Orbitron", sans-serif;">1.000</div></div>';
    div.innerHTML = htmlString.trim();
    audio_player.appendChild(div);
  },
};
...
var soundscape_player = {
  ...
  init: function () {
    ...
    for (i = 0; i < players.length; i++) {
      ...
      if (imgElms.length == 0) {
        ...
        console.log(
          'Soundscape Player: Found multiple images in player. First image element is assumed to be the soundscape.'
        );
      }
      ...
      if (audioElms.length == 0) {
        ...
        console.log(
          'Soundscape Player: Found multiple audio elements in player. First audio element is assumed to be the audio file.'
        );
      }
      ...
      axisWidth = player.getAttribute('data-axis-width')
        ? player.getAttribute('data-axis-width')
        : this.defaultAxisWidth;
      axisDivisionHeight = player.getAttribute('data-axis-division-height')
        ? player.getAttribute('data-axis-division-height')
        : this.defaultAxisDivisionHeight;
      axisSmoothing = player.getAttribute('data-axis-smoothing')
        ? player.getAttribute('data-axis-smoothing')
        : this.defaultAxisSmoothing;
  ...
  moveSoundscapes: function () {
    for (i = 0; i < this.playerIDs.length; i++) {
      ...
      img.src = viewerStyle.backgroundImage.replace(/url\(\"|\"\)$/gi, '');
  ...
  smoothAxis: function (range, baseDivision, allowedDecimals, distance) {
    if (distance == 0) return baseDivision;
    ...
    for (var i = 0; i <= distance; i++) {
      ...
      if (d1 > 0) {
        ...
        if (allowedDecimals.indexOf(decimal) > -1) return d1;
      }

      if (d2 > 0) {
        decimal = this.qoutientDecimal(range, d2, 4);
        if (allowedDecimals.indexOf(decimal) > -1) return d2;
      }
    }
  ...
  qoutientDecimal: function (dividend, divisor, precision) {
    ...
    if (precision === undefined) b = 1;
    else b = Math.pow(10, precision);
...
var setData = function () {
  const source = document.getElementById('audio-src');
  const sourceImg = document.getElementById('soundscape-img');
  const sourceUrl = data[audioIndex]?.player_clip_path?.replace(
    '/LUSTRE/sacmod/audio/sipecam/process_sipecam/cumulo_32/soundscape_player_data/',
    ''
  );
  const imgUrl = data[audioIndex]?.player_image_path?.replace(
    '/LUSTRE/sacmod/audio/sipecam/process_sipecam/cumulo_32/soundscape_player_data/',
    ''
  );
...
window.onload = function () {
  audioIndex = getUrlParameter('audio');
  setData();
